...
export const BOOT_PROMPT = [
  ...
  "- vibetui_eval(code)：在 $ui 活组件树上执行 JS —— 真 JS 对象+真函数，组件插入/删除/props 热换/监听每步立即上屏",
...
画布是一棵活组件树，你用 vibetui_eval 执行真 JS 操作它，每步立即上屏、立即可交互。
...
- 组件名：Button / Card / Row / Col / Space / Input / TextArea / InputNumber / Select /
...
- handler 是真闭包，可直接用 $agent.send / $ui / 本次 eval 里定义的函数；
  每次 eval 作用域独立，需跨 eval 复用的辅助函数可存进 $ui.data（如 $ui.data.fmt = (v) => ...）
