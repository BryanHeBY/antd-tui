...
提示符做成**主色徽章**:cwd 占一块 `colorPrimary` 底色(暗色派生出的深蓝)配 `colorPrimaryHover` 文字,左右各一格内边距的实心方块,硬边收尾——不用端帽字符(半圆/三角/半块都要么太小要么依赖字体),也不用又一档灰(灰阶之间差异太小、扫视时抓不住);主题覆盖主色时徽章跟着变。卡片底色仍分三档,从亮到暗:输入 `#1f1f1f`、agent 回复 `#1a1a1a`、输出 `#171717`。斜杠命令的结果不是纯文本行而是结构化的 `CommandRow`(标记 + 主体 + 参数提示 + 说明 + 备注),因此 `/help` 的命令名、`/session` 的当前会话、`/mode` 的当前模式都能各自上色,而不必靠猜字符串边界。
