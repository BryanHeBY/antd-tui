...
底色分四档,从亮到暗:提示符 chip `#2d2d2d`(cwd 单独占一块,让「在哪里」在整列卡片里对齐成一条视觉锚线)、输入 `#1f1f1f`、agent 回复 `#1a1a1a`、输出 `#171717`。斜杠命令的结果不是纯文本行而是结构化的 `CommandRow`(标记 + 主体 + 参数提示 + 说明 + 备注),因此 `/help` 的命令名、`/session` 的当前会话、`/mode` 的当前模式都能各自上色,而不必靠猜字符串边界。
